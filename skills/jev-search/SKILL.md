---
name: jev-search
description: Web search through Jev Search. Jev reads a plain-language request, picks the sources (Google, DuckDuckGo, Yandex, Hacker News, Reddit, GitHub, X, arXiv, YouTube, Wikipedia, IMDb, WeChat) and a time window, then ranks every result by relevance. Use it whenever the user needs current information from the web, asks what people are saying about something on Hacker News, Reddit or X, or wants links, papers, repos, videos or threads. Reach for it before scripting curl against a site's search API.
---
...
1. The built-in `WebSearch` tool, when the jev-search plugin is installed: every WebSearch call is answered by Jev and falls back to the built-in search only if Jev fails, so a plain WebSearch call is already a Jev search
2. The MCP tool `jev_search`, when you need to force `sources` or `window`, or when the plugin is not installed but the server was added with `claude mcp add` / `codex mcp add`
3. The CLI, which needs only Node 22 and installs nothing permanently
...
- The percentage is Jev's probability that the row is about the subject asked for, judged from title and snippet; it is not a measure of accuracy or authority
